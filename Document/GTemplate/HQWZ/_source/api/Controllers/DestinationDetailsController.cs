...
namespace HQWZ.Controllers
{
    public class DestinationDetailsController : Controller
    {
		...
		{
			return ExController.JsonNet(new { JsonData="xxx", Detinations="xxx" });
		}
		...
		public JsonResult LoadDContentList(*SearchEntityType* se, int page_g, int psize_g)
        {
		...
		public JsonResult SaveDDetail(*EntityType* entity)
        {
            if (entity.ID == 0)
            {
                #region 维护新增数据
		...
		public long DeleteDDetail(*EntityType* entity)
        {
