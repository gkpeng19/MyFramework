...
namespace HQWZ.Controllers
{
    public class VillageRoomController : Controller
    {
		...
		{
			return ExController.JsonNet(new { Villages="xxx" });
		}
		...
		public JsonResult LoadRoomList(*SearchEntityType* se, int page_g, int psize_g)
        {
		...
		public JsonResult SaveRoom(*EntityType* entity)
        {
            if (entity.ID == 0)
            {
                #region 维护新增数据
		...
		public long DeleteRoom(*EntityType* entity)
        {
