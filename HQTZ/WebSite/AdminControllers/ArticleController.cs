...
namespace HQWZ.Controllers
{
    ...
    {
        public JsonResult LoadArticleList(SearchModel se, int page_g, int psize_g)
        {
            ...
            var result = se.Load<HQ_Article>();
            return ExController.JsonNet(result);
        }


        [ValidateInput(false)]
        public JsonResult SaveArticle(HQ_Article entity)
        {
            if (entity.ID == 0)
            {
                #region 维护新增数据
        ...
        public long DeleteArticle(HQ_Article entity)
        {
